...
scope=api`,
      ...
      },
      {
        ...
        note: "entity_hotkey is null for tenants riding the shared platform miner — treat it as nullable.",
        runOp: "oauth.me",
      },
    ],
  },
  ...
  },
  {
    ...
    endpoints: [
      ...
      { method: "POST", path: "/v2/admin/apps", summary: "Register a new tenant app.", auth: "admin", request: `{ "name": "Acme", "slug": "acme", "entity_hotkey": "5F... (optional)", "network_api_key": "... (optional)", "scopes": ["api"] }`, response: `{ "app_id": "app_...", "client_id": "hsc_...", "client_secret": "hsk_...", "note": "store the secret now" }`, note: "Only name and slug are required. entity_hotkey and network_api_key are optional but both-or-neither (422 V2_APP_CREDENTIALS_MISMATCH on a half-supplied pair); omit both and the tenant rides the shared platform miner (requires V2_ENTITY_MINER_API_KEY on the deployment, else 422 V2_APP_NO_NETWORK_KEY)." },
